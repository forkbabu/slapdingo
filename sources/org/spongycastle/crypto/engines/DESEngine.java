...
public class DESEngine implements BlockCipher {
    ...
    private static final byte[] pc1 = {56, ByteBuffer.ZERO, 40, DocWriter.SPACE, 24, 16, 8, 0, 57, 49, 41, 33, 25, BidiOrder.WS, 9, 1, 58, 50, 42, DocWriter.QUOTE, 26, 18, 10, 2, 59, 51, 43, 35, DocWriter.GT, 54, 46, 38, 30, 22, BidiOrder.BN, 6, DocWriter.EQUALS, 53, 45, 37, 29, 21, BidiOrder.NSM, 5, DocWriter.LT, 52, 44, 36, 28, 20, BidiOrder.CS, 4, 27, 19, BidiOrder.AN, 3};
    private static final byte[] pc2 = {BidiOrder.NSM, 16, 10, 23, 0, 4, 2, 27, BidiOrder.BN, 5, 20, 9, 22, 18, BidiOrder.AN, 3, 25, 7, BidiOrder.B, 6, 26, 19, BidiOrder.CS, 1, 40, 51, 30, 36, 46, 54, 29, 39, 50, 44, DocWriter.SPACE, DocWriter.FORWARD, 43, ByteBuffer.ZERO, 38, 55, 33, 52, 45, 41, 49, 35, 28, 31};
    private static final byte[] totrot = {1, 2, 4, 6, 8, 10, BidiOrder.CS, BidiOrder.BN, BidiOrder.B, BidiOrder.WS, 19, 21, 23, 25, 27, 28};
    ...
    public void desFunc(int[] iArr, byte[] bArr, int i, byte[] bArr2, int i2) {
        byte b = ((bArr[i + 0] & UByte.MAX_VALUE) << 24) | ((bArr[i + 1] & UByte.MAX_VALUE) << 16) | ((bArr[i + 2] & UByte.MAX_VALUE) << 8) | (bArr[i + 3] & UByte.MAX_VALUE);
        byte b2 = ((bArr[i + 4] & UByte.MAX_VALUE) << 24) | ((bArr[i + 5] & UByte.MAX_VALUE) << 16) | ((bArr[i + 6] & UByte.MAX_VALUE) << 8) | (bArr[i + 7] & UByte.MAX_VALUE);
        byte b3 = ((b >>> 4) ^ b2) & BidiOrder.B;
        byte b4 = b2 ^ b3;
        byte b5 = b ^ (b3 << 4);
        byte b6 = ((b5 >>> 16) ^ b4) & UByte.MAX_VALUE;
        byte b7 = b4 ^ b6;
        byte b8 = b5 ^ (b6 << 16);
